...
class Processor:
    ...
    def get_HSI_patches(self, x, gt, ksize, stride=(1, 1), padding='reflect', indix=False):
        """
        extract HSI spectral-spatial (PyTorch版本，与原TensorFlow代码100%逻辑对齐)
        ...
        # ---------------------- 步骤2：边界填充（替换np.pad为PyTorch实现） ----------------------
        # 注意：PyTorch的F.pad参数顺序是从最后一个维度开始的
        # x: (H, W, C) -> pad顺序: (C前, C后, W左, W右, H上, H下)
        x_pad = F.pad(
            pt.from_numpy(x).float(),
            pad=(0, 0, pad_left, pad_right, pad_top, pad_down),
            mode=padding
        )
        # gt: (H, W) -> pad顺序: (W左, W右, H上, H下)
        gt_pad = F.pad(
            pt.from_numpy(gt).float(),
            pad=(pad_left, pad_right, pad_top, pad_down),
            mode=padding
        )

        n_row, n_clm, n_band = x_pad.shape

        # ---------------------- 步骤3：维度转换（TF NHWC -> PyTorch NCHW） ----------------------
        # 原TF: (1, H, W, C) -> PyTorch: (1, C, H, W)
        x_torch = x_pad.permute(2, 0, 1).unsqueeze(0)  # (1, C, H, W)
        y_torch = gt_pad.unsqueeze(0).unsqueeze(0)      # (1, 1, H, W)

        # ---------------------- 步骤4：提取图像patch（替换tf.extract_image_patches） ----------------------
        k_h, k_w = ksize
        s_h, s_w = stride

        # 使用Unfold提取patch（核心逻辑，与TF完全等效）
        unfold = pt.nn.Unfold(
            kernel_size=(k_h, k_w),
            stride=(s_h, s_w),
            padding=0,  # 已提前做了padding，这里用VALID模式
            dilation=1
        )

        # 提取x的patch: 输出形状 (1, C*k_h*k_w, H_out*W_out)
        x_patches_unfold = unfold(x_torch)
        # 提取y的patch: 输出形状 (1, 1*k_h*k_w, H_out*W_out)
        y_patches_unfold = unfold(y_torch)

        # 计算输出尺寸（与TF VALID模式计算公式完全一致）
        H_out = (n_row - k_h) // s_h + 1
        W_out = (n_clm - k_w) // s_w + 1

        # ---------------------- 步骤5：重塑为原代码格式（保持后续逻辑不变） ----------------------
        # 重塑x_patches为 (H_out*W_out, k_h, k_w, C)
        x_patches = x_patches_unfold.view(n_band, k_h, k_w, H_out, W_out)
        x_patches = x_patches.permute(3, 4, 1, 2, 0).contiguous()
        x_patches = x_patches.view(-1, k_h, k_w, n_band)

        # 重塑y_patches为 (H_out*W_out, k_h, k_w, 1)
        y_patches = y_patches_unfold.view(1, k_h, k_w, H_out, W_out)
        y_patches = y_patches.permute(3, 4, 1, 2, 0).contiguous()
        y_patches = y_patches.view(-1, k_h, k_w, 1)

        # ---------------------- 步骤6：提取中心标签并过滤背景（与原代码完全一致） ----------------------
        i_1, i_2 = int((ksize[0] - 1) // 2), int((ksize[1] - 1) // 2)
        y_center_label = y_patches[:, i_1, i_2, :].view(-1)
        nonzero_index = pt.nonzero(y_center_label, as_tuple=True)

        # 过滤非零样本
        x_patches_nonzero = x_patches[nonzero_index]
        y_patches_nonzero = y_center_label[nonzero_index]

        # ---------------------- 步骤7：转换为numpy数组（与原函数输出格式一致） ----------------------
        x_patches_nonzero = x_patches_nonzero.numpy()
        y_patches_nonzero = y_patches_nonzero.numpy()

        if indix is True:
            # 转换索引为numpy元组，与原代码一致
            nonzero_index_np = tuple(idx.numpy() for idx in nonzero_index)
            return x_patches_nonzero, y_patches_nonzero, nonzero_index_np
